...
def got_tweet(tweet_id, text, created_at):
    ...
    # Compute the company
    company = ""
    if companies[0].lower() in text.lower():
        company = "McDonalds"
    elif companies[2].lower() in text.lower():
        company = "Chipotle"
    elif companies[4].lower() in text.lower():
        company = "Microsoft"
    elif companies[6].lower() in text.lower():
        company = "Fedex"
    elif companies[8].lower() in text.lower():
        company = "Disney"
    elif companies[10].lower() in text.lower() or companies[11].lower() in text.lower():
        company = "Tesla"
    elif companies[13].lower() in text.lower() or companies[14].lower() in text.lower():
        company = "Google"
    elif companies[16].lower() in text.lower():
        company = "Facebook"
    elif companies[18].lower() in text.lower():
        company = "Amazon"
    if company == "":
        return "No company"
